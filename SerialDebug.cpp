...
SerialDebug::SerialDebug(LEVEL level= ALL)
{
   debug_level=level;
}
...
void SerialDebug::print(String Header, String Message,byte lvl) {

if ( lvl>=debug_level ) {
		uint32_t now = millis();
		Serial.printf("%07u.%03u: ", now / 1000, now % 1000);
		Serial.print(" => ");
		Serial.print(Header);
		Serial.print(" = ");
 ...
 void SerialDebug::setDebugLevel(LEVEL level){
	 debug_level=level;
 }
